...
EasyJ_Checker.JS_PORT = function(text) {
...
EasyJ_Checker.JS_BUTTON = function(text) {
...
EasyJ_Checker.ANALOG_PORT = function(text) {
	var btn = parseInt(text);
	if (isNaN(btn)) {
		return null;
	}
...
EasyJ_Checker.DIGITAL_PORT = function(text) {
	var btn = parseInt(text);
	if (isNaN(btn)) {
		return null;
	}
...
EasyJ_Checker.PWM_PORT = function(text) {
	var btn = parseInt(text);
	if (isNaN(btn)) {
		return null;
	}
...
EasyJ_Checker.RELAY_PORT = function(text) {
	var btn = parseInt(text);
	if (isNaN(btn)) {
		return null;
	}
...
EasyJ_Checker.SOLENOID_PORT = function(text) {
	var btn = parseInt(text);
	if (isNaN(btn)) {
		return null;
	}
...
EasyJ_Checker.IS_NUM = function(text) {
...
EasyJ_Checker.IS_NUMF = function(text) {
...
EasyJ_Checker.EnsureNotTop_Init = function() {
  if (!this.workspace) {
...
EasyJ_Checker.PickWarning = function(block, warnings) {
  if (!$.isArray(warnings))
    warnings = [warnings];
...
EasyJ_Checker.EnsureVariablesExist = function(block) {
...
EasyJ_Checker.EnsureNotOrphaned = function(block) {
