...
class Gen_compressed(threading.Thread):
  ...
  def gen_generator(self, language):
    ...
    filenames = glob.glob(os.path.join(path_to_blockly, 'generators', language, '*.js'))
    gen = os.path.join(path_to_blockly, 'generators', language + '.js')
    if(os.path.isfile(gen)):
      filenames.insert(0, gen)

    filenames.extend(glob.glob(os.path.join(path_to_blockly_ext, 'generators', language, '*.js')))
    gen = os.path.join(path_to_blockly_ext, 'generators', language + '.js')
    if(os.path.isfile(gen)):
      filenames.insert(0, gen)
    for filename in filenames:
      f = open(filename)
...
class Gen_langfiles(threading.Thread):
  ...
  def run(self):
    ...
    if self._rebuild([os.path.join(path_to_blockly, 'msg', 'messages.js')],
                     [os.path.join(output_dir, 'msg', 'json', f) for f in
                      ['en.json', 'qqq.json', 'synonyms.json']]):
      # make sure the directory exists
      mkdir_p(os.path.join(output_dir,'msg','json'));
      try:
        subprocess.check_call([
            'python',
            os.path.join(path_to_blockly, 'i18n', 'js_to_json.py'),
            '--input_file', os.path.join(path_to_blockly,'msg','messages.js'),
            '--output_dir', os.path.join(output_dir,'msg','json'),
            '--quiet'])
    ...
    try:
      # make sure the directory exists
      mkdir_p(os.path.join(output_dir,'msg','js'));
      # Use create_messages.py to create .js files from .json files.
      cmd = [
          'python',
          os.path.join(path_to_blockly, 'i18n', 'create_messages.py'),
          '--source_lang_file', os.path.join(path_to_blockly, 'msg', 'json', 'en.json'),
          '--source_synonym_file', os.path.join(path_to_blockly, 'msg', 'json', 'synonyms.json'),
          '--key_file', os.path.join(path_to_blockly, 'msg', 'json', 'keys.json'),
          '--output_dir', os.path.join(output_dir, 'msg', 'js'),
          '--quiet']
      json_files = glob.glob(os.path.join(path_to_blockly, 'msg', 'json', '*.json'))
...
if __name__ == '__main__':
  try:
    calcdeps = import_path(os.path.join(path_to_closure, 'closure', 'bin', 'calcdeps.py'))
  except ImportError:
...
https://developers.google.com/blockly/hacking/closure""")
  ...
  search_paths = calcdeps.ExpandDirectories(
      [os.path.join(path_to_blockly, 'core'),
