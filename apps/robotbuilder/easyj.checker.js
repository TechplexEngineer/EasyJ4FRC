...
EasyJ.Checker.JS_PORT = function(text) {
	...
	if (port < 0 || port > 5) { //6 joystick ports
		return null;
	}
	return port;
};
...
EasyJ.Checker.ANALOG_PORT = function(text) {
	...
	if (btn < 0 || btn > 3) { //12 analog channels?
		return null;
	}
	return btn;
};
...
EasyJ.Checker.DIGITAL_PORT = function(text) {
	...
	if (btn < 0 || btn > 9) { //12 digital channels?
		return null;
	}
	return btn;
};
...
EasyJ.Checker.RELAY_PORT = function(text) {
	...
	if (btn < 0 || btn > 3) { //10 RELAY Ports?
		return null;
	}
	return btn;
};
EasyJ.Checker.PCM_CHNL = function(text){
	var btn = parseInt(text);
	if(isNaN(btn)){
		return null;
	}
...
EasyJ.Checker.SOLENOID_PORT = function(text) {
	...
	if (btn < 0 || btn > 7) { //10 SOLENOID Ports?
		return null;
	}
	return btn;
};
